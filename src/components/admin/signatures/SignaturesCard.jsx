...
// ══════════════════════════════════════════════════════════════
export function SignaturesCard({ schoolId, peutGererRoles, toast, comptes = [] }) {
  const s = useSignaturesCard({ schoolId, toast, comptes });
  ...
  return (
    <div style={{ background: "var(--lc-surface, #fff)", borderRadius: 14, padding: "20px 22px", marginBottom: 20, border: "1px solid #e2e8f0" }}>
      ...
      <p style={{ margin: "3px 0 12px", fontSize: 12, color: "#64748b" }}>
        ...
        Le responsable du poste s'imprime sous son titre — ou, si le compte qui imprime a son propre nom
        (poste à plusieurs comptes), ce nom-là. Un second signataire (visa) peut s'ajouter.
