  ...
  return (
    <>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:8,marginBottom:12}}>
        ...
        {promoRes.nonConcernes>0&&<div style={{background:"#f3f4f6",borderRadius:8,padding:"8px 12px",textAlign:"center"}}>
          <div style={{fontSize:20,fontWeight:900,color:"#6b7280"}}>{promoRes.nonConcernes}</div>
          <div style={{fontSize:11,color:"#6b7280"}}>➖ Non concernés</div>
        </div>}
      ...
      {promoRes.examens>0&&<p style={{margin:"0 0 12px",padding:"8px 12px",background:"#e0ebf8",borderRadius:8,fontSize:11,color:"#1e40af"}}>
        ...
        {promoRes.classesExamen?.length>0?` (${promoRes.classesExamen.join(", ")})`:""} — <strong>non touchés par la promotion</strong>.
        Leur passage se joue au CEE, au BEPC ou au BAC, pas sur nos moyennes : il se fait avec le
        « Passage des admis », une fois les résultats saisis.
      </p>}
      {promoRes.nonConcernes>0&&<p style={{margin:"0 0 12px",fontSize:11,color:"#6b7280"}}>
        ➖ {promoRes.nonConcernes} élève(s) non concerné(s) : inscrits après la clôture de {promoRes.annee},
        ou déjà changés de classe depuis. Ils ne sont pas touchés.
      </p>}
