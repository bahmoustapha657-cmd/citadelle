...
export function NotificationsTab({ schoolId, schoolInfo, toast, sec, lbl }) {
  ...
  return (
    <div style={sec}>
      ...
      {DECLENCHEURS.map((d) => (
        <label key={d.cle} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 0", borderBottom: "1px solid var(--lc-border)", cursor: "pointer" }}>
          <input type="checkbox" checked={prefs[d.cle]} onChange={() => toggle(d.cle)} style={{ marginTop: 3, width: 18, height: 18, cursor: "pointer" }} />
          <span>
      ...
      <button onClick={enregistrer} disabled={enCours}
        ...
        {enCours ? "Enregistrement…" : "💾 Enregistrer les notifications"}
      </button>
    </div>
  );
}
