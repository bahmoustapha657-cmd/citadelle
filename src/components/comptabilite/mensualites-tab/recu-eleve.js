...
// reste à payer. `versement` (facultatif) : le paiement qui vient d'être
// encaissé — { date, total, lignes: [{ libelle, montant }] } — mis en avant
// sur le reçu. À appeler dans le geste de l'utilisateur (ouverture de fenêtre).
export function imprimerRecuEleve({ eleve, tarifsClasses = [], moisAnnee = [], schoolInfo = {}, format = "a4", versement = null }) {
  ...
  const options = { versement, resteAPayer: getEleveSolde(eleve, moisAnnee, tarifsClasses) };
