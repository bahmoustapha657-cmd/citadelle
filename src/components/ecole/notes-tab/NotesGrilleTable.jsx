...
// Tableau de saisie générique : lignes (élèves OU matières selon le mode)
// × colonnes (matières OU périodes). Moyenne par ligne sur les colonnes
// renseignées. Cellules éditables (canCreate).
export function NotesGrilleTable({
  lignes, colonnes, ligneHeader = "Élève", maxNote, canCreate,
  valeurCellule, cleCellule, grilleChanges, setGrilleChanges,
}) {
  if (lignes.length === 0) return <Vide icone="📝" msg="Rien à afficher" />;
  if (colonnes.length === 0) return <Vide icone="📚" msg="Sélectionnez un élément à saisir" />;
  return (
    <div style={{ overflow: "auto", maxHeight: "calc(100vh - 360px)", minHeight: 280, border: "1px solid var(--lc-border)", borderRadius: 8 }}>
      <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, minWidth: 600 }}>
        ...
        <tbody>
          {lignes.map((ligne, ri) => {
            const nbRemplies = colonnes.filter(col => valeurCellule(ligne.key, col) !== "").length;
            const moy = colonnes.reduce((s, col) => {
              const v = Number(valeurCellule(ligne.key, col));
              return s + (isNaN(v) ? 0 : v);
            }, 0) / (nbRemplies || 1);
            const rowBg = ri % 2 === 0 ? "#fff" : "#f8fafc";
            return (
              <tr key={ligne.key} style={{ background: rowBg, borderBottom: "1px solid #f1f5f9" }}>
                <td style={{ padding: "6px 12px", fontSize: 12, fontWeight: 700, color: C.blueDark, whiteSpace: "nowrap", position: "sticky", left: 0, zIndex: 1, background: rowBg, boxShadow: "inset -1px 0 0 var(--lc-border-soft)" }}>
                  {ligne.label}
                  {ligne.sub ? <span style={{ fontSize: 10, color: "#94a3b8", marginLeft: 6 }}>{ligne.sub}</span> : null}
                  {nbRemplies > 0 && moy > 0 && <span style={{ marginLeft: 8, fontSize: 11, fontWeight: 900, ...couleurNote(moy, maxNote) }}>{moy.toFixed(1)}</span>}
                </td>
                {colonnes.map(col => {
                  const key = cleCellule(ligne.key, col);
                  const val = valeurCellule(ligne.key, col);
