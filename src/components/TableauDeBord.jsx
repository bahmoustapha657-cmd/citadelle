...
function TableauDeBord({annee}) {
  ...
  const envoyerDemande = async () => {
    ...
    try {
      ...
      // Ne pas fermer le formulaire — montrer le succes dans le panneau
      setTimeout(()=>{ setDemandeSucces(false); setDemandeOuverte(false); setDemandeForm({operateur:"Orange Money",telephone:"",reference:""}); }, 5000);
    } catch(e){
      console.error(e);
      alert("Erreur lors de l'envoi. Verifiez votre connexion et reessayez.");
    }
  ...
  // Taux de paiement des mensualites
  const calcTauxPaiement = (eleves) => {
    ...
    const payes = eleves.reduce((s,e)=>s+Object.values(e.mens||{}).filter(v=>v==="Paye").length,0);
    ...
    <div style={{padding:"22px 26px",maxWidth:1200}}>
      ...
      <div style={{marginBottom:24,display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
        <div>
          ...
          <p style={{margin:"4px 0 0",fontSize:13,color:"#6b7280"}}>Annee {annee||getAnnee()} - Vue consolidee</p>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12,marginBottom:24}}>
        <KPI label="Eleves actifs"  value={totalEleves} icon="EL" sub={`${elevesC.filter(e=>e.statut==="Actif").length} college - ${elevesL.filter(e=>e.statut==="Actif").length} lycee - ${elevesP.filter(e=>e.statut==="Actif").length} primaire`}/>
        <KPI label="Enseignants"    value={totalEns}    icon="ENS" sub={`${ensC.length}C - ${ensL.length}L - ${ensP.length}P`}/>
        <KPI label="Taux paiement"  value={`${tauxPay}%`} icon="PAY" color="green" sub="Mensualites toutes sections"/>
        <KPI label="Solde treso."   value={fmt(solde)} icon="GNF" color={solde>=0?"green":"white"} sub={`Rec: ${fmt(totalRec)} / Dep: ${fmt(totalDep)}`}/>
        <KPI label="Masse salariale" value={fmt(masseSal)} icon="SAL" sub={`${salMois.length} lignes - ${moisActuel}`}/>
        <KPI label="Absences saisies" value={totalAbs} icon="ABS" sub="Toutes sections"/>
      </div>

      {/* Graphiques */}
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:16,marginBottom:20}}>
        {/* Répartition eleves par classe */}
        <Card><div style={{padding:"16px 18px"}}>
          <p style={{margin:"0 0 14px",fontWeight:800,fontSize:13,color:c1}}>Repartition des eleves par section</p>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={[
              {section:"College",Actifs:elevesC.filter(e=>e.statut==="Actif").length,Inactifs:elevesC.filter(e=>e.statut!=="Actif").length},
              {section:"Lycee",Actifs:elevesL.filter(e=>e.statut==="Actif").length,Inactifs:elevesL.filter(e=>e.statut!=="Actif").length},
        ...
        <Card><div style={{padding:"16px 18px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16,marginTop:8}}>
            {[["College",tauxPayC],["Lycee",tauxPayL],["Primaire",tauxPayP]].map(([label,taux])=>(
              <div key={label}>
      ...
      {/* ── Tendances annuelles ── */}
      {(()=>{
        // Taux paiement mois par mois pour tous les eleves
        const tousEleves = [...elevesC,...elevesL,...elevesP];
        const dataTendance = moisAnnee.map(m=>{
          const payesMois = tousEleves.filter(e=>(e.mens||{})[m]==="Paye").length;
        ...
        return (
          <Card style={{marginBottom:16}}><div style={{padding:"16px 18px"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              ...
              {/* Courbe absences */}
              <div>
                <p style={{fontSize:11,fontWeight:700,color:"#64748b",margin:"0 0 8px",textTransform:"uppercase",letterSpacing:"0.06em"}}>Absences enregistrees</p>
                <ResponsiveContainer width="100%" height={160}>
                  <LineChart data={dataTendance} margin={{top:4,right:8,left:-20,bottom:0}}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#e0ebf8"/>
                    <XAxis dataKey="mois" tick={{fontSize:10}}/>
      ...
      {/* Evenements a venir + Alertes */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <Card><div style={{padding:"16px 18px"}}>
          <p style={{margin:"0 0 12px",fontWeight:800,fontSize:13,color:c1}}>Prochains evenements</p>
          {evAVenir.length===0
            ? <Vide icone="CAL" msg="Aucun evenement planifie"/>
            : evAVenir.map(ev=>(
              <div key={ev._id} style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:10,padding:"8px 10px",background:"#f8fafc",borderRadius:8,borderLeft:`3px solid ${TYPE_COLORS[ev.type]||"#6366f1"}`}}>
                ...
                <Badge color={ev.type==="exam"?"red":ev.type==="conge"?"green":ev.type==="reunion"?"orange":"blue"}>{ev.type||"evenement"}</Badge>
              </div>
            ))
          }
        </div></Card>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            ...
            {solde < 0 && (
              <div style={{padding:"10px 12px",background:"#fef2f2",borderRadius:8,borderLeft:"3px solid #ef4444",fontSize:12}}>
                <span style={{fontWeight:800,color:"#991b1b"}}>Solde de tresorerie negatif</span>
                <div style={{color:"#991b1b",marginTop:2}}>Solde : {fmt(solde)} GNF</div>
              </div>
            )}
            {masseSal > totalRec * 0.7 && masseSal > 0 && (
              <div style={{padding:"10px 12px",background:"#fef3e0",borderRadius:8,borderLeft:"3px solid #f59e0b",fontSize:12}}>
                <span style={{fontWeight:800,color:"#92400e"}}>Masse salariale elevee</span>
            ...
            {tauxPay >= 60 && solde >= 0 && (
              <div style={{padding:"10px 12px",background:"#f0fdf8",borderRadius:8,borderLeft:`3px solid ${c2}`,fontSize:12}}>
                ...
                <div style={{color:"#065f46",marginTop:2}}>Tresorerie positive et taux de paiement satisfaisant.</div>
              </div>
            )}
          </div>
        </div></Card>
      ...
      {planInfo && (
        <div style={{marginTop:24}}>
          ...
          {/* Banniere expiration / periode de grace / limite */}
          {(planInfo.planEstExpire || planInfo.enPeriodeGrace || planInfo.joursRestants!==null&&planInfo.joursRestants<=30 ||
            ...
            <div style={{
              ...
              <span style={{fontSize:22}}>
                {planInfo.planEstExpire?"!":planInfo.enPeriodeGrace?"!":planInfo.joursRestants<=7?"!":"i"}
              </span>
              <div style={{flex:1}}>
                <p style={{margin:0,fontWeight:800,fontSize:13,color:planInfo.planEstExpire?"#991b1b":planInfo.enPeriodeGrace?"#c2410c":"#92400e"}}>
                  {planInfo.planEstExpire
                    ? "Abonnement expire - acces limite a 50 eleves"
                    : planInfo.enPeriodeGrace
                      ? `Periode de grace - encore ${planInfo.joursGrace} jour(s) d'acces complet`
                      : planInfo.joursRestants!==null&&planInfo.joursRestants<=30
                        ...
                        : `Plan Gratuit : ${planInfo.totalElevesActifs}/50 eleves - bientot a la limite`}
                </p>
                <p style={{margin:"2px 0 0",fontSize:11,color:"#6b7280"}}>
                  {planInfo.enPeriodeGrace
                    ? "Renouvelez votre abonnement avant la fin de la periode de grace pour ne pas perdre l'acces."
                    : "Souscrivez un abonnement pour continuer a inscrire des eleves sans limite."}
                </p>
              </div>
            </div>
          )}
          ...
          {demandeSucces && (
            <div style={{background:"#d1fae5",border:"1px solid #6ee7b7",borderRadius:10,padding:"12px 18px",marginBottom:16,fontSize:13,color:"#065f46",fontWeight:700}}>
              OK Demande envoyee ! L'equipe EduGest va traiter votre demande et activer votre abonnement.
            </div>
          )}
          ...
          <div style={{background:"#fff",borderRadius:14,boxShadow:"0 2px 16px rgba(0,32,80,0.07)",overflow:"hidden"}}>
              ...
              <div>
                ...
                <p style={{margin:"3px 0 0",fontSize:12,color:"#6b7280"}}>
                  {planInfo.planCourant==="gratuit"
                    ? `${planInfo.totalElevesActifs}/50 eleves actifs - gratuit jusqu'a 50`
                    : planInfo.planEstExpire
                      ? "Expire - limite a 50 eleves"
                      : planInfo.enPeriodeGrace
                        ? `Periode de grace - ${planInfo.joursGrace} jour(s) restant(s)`
                        : `${planInfo.totalElevesActifs} eleves actifs - expire le ${new Date(planInfo.planExpiry).toLocaleDateString("fr-FR")}`}
                </p>
              </div>
            ...
            {demandeOuverte && (
              <div style={{padding:"20px 24px"}}>
                <p style={{margin:"0 0 16px",fontSize:13,color:"#374151"}}>Remplissez ce formulaire apres avoir effectue votre paiement mobile. L'equipe EduGest validera et activera votre abonnement sous 24h.</p>

                {/* Choix du plan */}
                <label style={{display:"block",fontSize:11,fontWeight:700,color:C.blue,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:8}}>Plan souhaite</label>
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:18}}>
                  {Object.entries(PLANS).filter(([k])=>k!=="gratuit").map(([key,info])=>(
                    <button key={key} onClick={()=>setDemandePlan(key)}
                      ...
                      <div style={{fontSize:11,color:"#6b7280",marginTop:2}}>{info.eleveLimit===Infinity?"Illimite":`<= ${info.eleveLimit} eleves`}</div>
                    </button>
                  ))}
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
                  <div>
                    <label style={{display:"block",fontSize:11,fontWeight:700,color:"#374151",marginBottom:4}}>Operateur Mobile Money</label>
                  ...
                  <div>
                    <label style={{display:"block",fontSize:11,fontWeight:700,color:"#374151",marginBottom:4}}>Numero de telephone</label>
                ...
                <div style={{marginBottom:18}}>
                  <label style={{display:"block",fontSize:11,fontWeight:700,color:"#374151",marginBottom:4}}>Reference du paiement</label>
                ...
                {demandeSucces ? (
                  <div style={{background:"#d1fae5",border:"2px solid #6ee7b7",borderRadius:12,padding:"20px",textAlign:"center"}}>
                    ...
                    <p style={{margin:"0 0 4px",fontWeight:800,fontSize:15,color:"#065f46"}}>Demande envoyee avec succes !</p>
                    <p style={{margin:0,fontSize:12,color:"#047857"}}>L'equipe EduGest va verifier votre paiement et activer votre abonnement <strong>{PLANS[demandePlan]?.label}</strong> sous 24h.</p>
                  </div>
                ) : (
...
// ══════════════════════════════════════════════════════════════
//  PARAMETRES MATRICULE (sous-composant)
// ══════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════
