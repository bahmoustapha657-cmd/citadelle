...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "reset_password") {
      ...
      // FILET DE SÉCURITÉ — se réinitialiser SOI-MÊME par la voie
      // administrateur déconnecte : `auth.admin.updateUserById` révoque les
      // jetons de rafraîchissement du compte visé, d'où les 401 en série et le
      // rafraîchissement en 400 déjà observés en production.
      // Le client N'ARRIVE NORMALEMENT PLUS ICI pour son propre compte : il
      // bascule sur `auth.updateUser` (account-manage-supabase.js), qui fait le
      // même changement mais rend les nouveaux jetons au navigateur, donc
      // conserve la session. Ce refus ne protège que les appelants anciens ou
      // directs.
      if (cible.user_id === user.id) {
        return json({
          error: "Changez votre propre mot de passe depuis l'écran « Changer mon mot de passe » : cette voie déconnecterait votre session.",
        }, 409);
      }
